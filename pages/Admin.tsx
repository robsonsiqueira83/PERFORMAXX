...
import { Trash2, Edit, Plus, Settings, Loader2, Link as LinkIcon, Copy, X, CheckCircle, AlertCircle, Shirt, ExternalLink, Globe, Target, Upload, Users } from 'lucide-react';
...
type ModalType = 'none' | 'edit_team' | 'edit_category' | 'alert_error' | 'alert_success';
...
const Admin: React.FC<AdminProps> = ({ userRole, currentTeamId }) => {
  ...
  const copyToClipboard = (text: string) => {
      ...
      setModalMessage('Link público copiado!');
  ...
  const handleDeleteTeam = async (id: string, name: string) => {
      if (!window.confirm(`AVISO: Deseja realmente excluir a equipe "${name}"? Todos os atletas vinculados serão removidos do painel.`)) return;
      await deleteTeam(id);
      refreshData();
  };
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          ...
          } catch (err) { alert("Erro logo"); } finally { setUploading(false); }
      }
  };
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in">
      ...
      <div className="p-8">
        {activeTab === 'teams' && (
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => { setTargetId(null); setFormData({name:'', logoUrl:''}); setModalType('edit_team'); }} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg"><Plus size={16}/> Nova Equipe</button>
                </div>
                            ...
                            <div key={team.id} className="p-6 border border-gray-100 rounded-3xl bg-white shadow-sm hover:shadow-md transition-all space-y-6">
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                    <div className="flex items-center gap-5">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 mt-2">
                                                ...
                                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter flex items-center gap-1"><Users size={12}/> {members.length} Colaboradores</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex gap-2 w-full md:w-auto">
                                        ...
                                        <button onClick={() => handleDeleteTeam(team.id, team.name)} className="bg-red-50 text-red-500 hover:bg-red-100 p-3 rounded-xl border border-red-100 transition-all"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                                ...
                                <div className="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-50 flex flex-col md:flex-row items-center justify-between gap-4">
                                    <div className="flex items-center gap-3">
                                        <div className="bg-white p-2 rounded-xl text-indigo-600 shadow-sm"><Globe size={16}/></div>
                                        <div className="text-[10px] font-black uppercase text-indigo-800 tracking-widest">Painel Público: <span className="text-indigo-400 font-mono text-[9px]">{pubLink}</span></div>
                                    </div>
                                    <button onClick={() => copyToClipboard(pubLink)} className="bg-white text-indigo-600 p-2 rounded-lg border border-indigo-200 hover:bg-indigo-100 transition-all shadow-sm"><Copy size={14}/></button>
                                </div>

                                <div className="space-y-2">
                                    <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-2">Staff do Time</span>
                                    <div className="flex flex-wrap gap-2">
                                        {members.map(u => (
                                            <div key={u.id} className="flex items-center gap-2 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-xl">
                                                {u.avatarUrl ? <img src={u.avatarUrl} className="w-5 h-5 rounded-full object-cover" /> : <div className="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center text-[8px] font-bold">{u.name.charAt(0)}</div>}
                                                <span className="text-[10px] font-bold text-gray-700">{u.name.split(' ')[0]}</span>
                                                <span className="text-[8px] font-black text-gray-400 uppercase tracking-tighter bg-white px-1.5 rounded border">{u.role}</span>
                                            </div>
                                        ))}
        ...
        {activeTab === 'categories' && (
            <div className="space-y-6">
                 <div className="flex justify-between items-center">
                    <h3 className="text-sm font-black text-gray-800 uppercase tracking-widest flex items-center gap-2"><Target size={16} className="text-emerald-500"/> Categorias da Equipe</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {categories.map(cat => (
                        <div key={cat.id} className="p-5 border border-gray-100 rounded-2xl flex justify-between items-center bg-white shadow-sm hover:border-emerald-200 transition-all">
                            ...
                            <div className="flex gap-1">
                                ...
                                <button onClick={async () => { if(window.confirm('Excluir categoria?')) { await deleteCategory(cat.id); refreshData(); } }} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={14}/></button>
                            </div>
                        </div>
                    ))}
                </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl animate-slide-up">
              <div className="flex justify-between items-center mb-8 border-b pb-4">
                ...
                <button onClick={() => setModalType('none')}><X className="text-gray-300 hover:text-red-500"/></button>
              </div>
              ...
              <div className="space-y-6">
                  ...
                  <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase mb-1.5 ml-1">Nome da {modalType === 'edit_team' ? 'Equipe' : 'Categoria'}</label>
              ...
              <div className="flex gap-3 mt-8 pt-4 border-t border-gray-50">
                  ...
                  <button onClick={modalType === 'edit_team' ? handleSaveTeam : handleSaveCategory} disabled={uploading || !formData.name} className="flex-[2] bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-[10px] hover:bg-indigo-700 disabled:opacity-50">Salvar Alterações</button>
              </div>
           </div>
        </div>
      )}

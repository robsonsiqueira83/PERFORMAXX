...
import { 
  getAthletes, getTrainingEntries, getTrainingSessions, getTeams, getEvaluationSessions,
  getTechnicalEvaluations, getPhysicalEvaluations 
} from '../services/storageService';
import { TrainingEntry, Athlete, EvaluationSession, TechnicalEvaluation, PhysicalEvaluation, formatDateSafe } from '../types';
import { 
...
import { 
  ...
  TrendingUp, Activity, Target, Filter, MousePointer2, ClipboardCheck, Info, Timer, Eye, AlertCircle
} from 'lucide-react';
...
const PublicAthleteProfile: React.FC = () => {
  ...
  useEffect(() => {
     const load = async () => {
         ...
         if (foundAthlete) {
             ...
             // Load details for snapshots tab
  ...
  const realtimeHistory = useMemo(() => {
      ...
      return matchEntries.map(e => {
          ...
          try {
              ...
              return { id: e.id, date: session?.date || '--', eventsCount: 0, avgImpact: 0, impactLabel: 'erro' };
          }
  ...
  const radarAggregatedPhys = useMemo(() => {
    ...
    return groups.map(g => {
        ...
        return { subject: g.name, A: Number(score.toFixed(1)), fullMark: 100 };
    });
  }, [filteredPhysEvals]);
              ...
              <div className="flex flex-col md:flex-row items-center gap-8 flex-1">
                  ...
                  <div className="flex-1 min-w-0 text-center md:text-left">
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-2">
                       ...
                       <span className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 text-[10px] px-2 py-1 rounded font-black uppercase tracking-widest">{athlete.categoryId ? 'Categoria Definida' : 'Sem Categoria'}</span>
                    </div>
                  </div>
              </div>
                  ...
                  <div className="bg-white dark:bg-darkCard p-8 rounded-[40px] border border-gray-100 dark:border-darkBorder shadow-sm flex flex-col h-full">
                              ...
                              <LineChart 
                                  ...
                                  onClick={(data: any) => {
                                      if (data && data.activePayload) {
                                          setFilterTimeBlock(data.activePayload[0].payload.raw);
                                      }
                                  }}
                              >
                                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#334155" />
                      ...
                      {filterTimeBlock !== null && (
                        <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl flex justify-between items-center animate-fade-in">
                            <span className="text-xs font-bold text-blue-700 dark:text-blue-300">Filtro Temporal Ativo: Minutos {Math.floor(filterTimeBlock/60)} a {Math.floor(filterTimeBlock/60) + 5}</span>
                            <button onClick={() => setFilterTimeBlock(null)} className="text-[10px] font-black text-blue-500 uppercase hover:text-blue-800 dark:hover:text-blue-300 transition-colors">Remover</button>
                        </div>
                      )}
                  </div>
              </div>
                  ...
                  <div className="divide-y divide-gray-50 dark:divide-darkBorder">
                      {realtimeHistory.length > 0 ? realtimeHistory.map(entry => (
                          <div key={entry.id} className="p-6 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-indigo-900/10 transition-all border-l-4 border-transparent hover:border-indigo-600">
                              ...
                              <div className="flex items-center gap-2">
                                  <Link to={`/p/athlete/${athleteId}/eval-view/${entry.id}`} className="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/10 text-indigo-600 dark:text-indigo-400 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-100 dark:hover:bg-indigo-900/20 transition-all shadow-sm"><Eye size={16}/> Detalhes</Link>
                              </div>
                          </div>
                          ...
                          <div key={ev.id} className="p-6 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-indigo-900/10 transition-all border-l-4 border-transparent hover:border-emerald-600">
                              ...
                              <div className="flex items-center gap-2">
                                  <Link 
                                    ...
                                    className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 dark:text-emerald-400 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100 dark:hover:bg-emerald-900/20 transition-all shadow-sm"
                                  >

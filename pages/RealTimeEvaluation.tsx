...
import { ArrowLeft, Play, Pause, XCircle, CheckCircle, StopCircle, Flag, UserPlus, Users, X, Plus, Search, Loader2, AlertTriangle, AlertCircle, RefreshCw, Activity, Rocket, Shield, Zap, ShieldAlert } from 'lucide-react';
...
const PHASE_ACTIONS: Record<GamePhase, string[]> = {
    'OFENSIVA': ['Apoio', 'Amplitude', 'Manutenção da posse', 'Entrelinhas', 'Profundidade', 'Penetração'],
...
const RealTimeEvaluation: React.FC = () => {
  ...
  const handleAddAthlete = (ath: Athlete) => {
      if (activeAthletes.find(a => a.id === ath.id)) return;
      setActiveAthletes(prev => [...prev, ath]);
      setSessionLogs(prev => ({ ...prev, [ath.id]: [] }));
  ...
  const handleFinishSession = async () => {
      ...
      for (const ath of activeAthletes) {
          ...
          await saveTrainingSession({
              id: sessionId, teamId: ath.teamId, categoryId: ath.categoryId!,
              date: today,
              description: `RealTime: ${logs.length} ações. Impacto: ${semanticImpact.replace('_', ' ')}`
          });

          const normalizedRadarScore = Math.max(0, Math.min(10, 5 + (avgScore * 3.33)));

          await saveTrainingEntry({
              id: uuidv4(), sessionId, athleteId: ath.id,
              ...
              notes: JSON.stringify({ type: 'TACTICAL_ANALYSIS_V2', totalScore, avgScore, impact: semanticImpact, events: logs })
          });
      }
  ...
  const filteredSearchAthletes = useMemo(() => {
      const activeIds = new Set(activeAthletes.map(a => a.id));
      return allAthletes.filter(a => !activeIds.has(a.id) && a.name.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [allAthletes, activeAthletes, searchTerm]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-darkBase pb-40 transition-colors">
      <div className="bg-white dark:bg-darkCard p-4 shadow-sm border-b border-gray-100 dark:border-darkBorder flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-3">
              <button onClick={() => navigate(`/athletes/${id}`)} className="text-gray-500 dark:text-gray-400 hover:text-blue-600"><ArrowLeft size={24} /></button>
              {currentAthlete && (
                  <div className="flex items-center gap-3">
                      {currentAthlete.photoUrl ? <img src={currentAthlete.photoUrl} className="w-10 h-10 rounded-full object-cover border-2 border-white dark:border-darkBorder" /> : <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-darkInput flex items-center justify-center font-bold text-blue-600 dark:text-blue-400 text-sm border dark:border-darkBorder">{currentAthlete.name.charAt(0)}</div>}
                      <div>
          ...
          <div className="flex items-center gap-2">
              <button onClick={() => setFieldFlipped(!fieldFlipped)} className="p-2.5 bg-gray-100 dark:bg-darkInput text-gray-500 dark:text-gray-400 hover:text-indigo-600 rounded-xl transition-all" title="Inverter Campo"><RefreshCw size={20}/></button>
              <div className={`flex flex-col items-center px-4 py-1 rounded-xl border-2 transition-all ${isRunning ? 'bg-red-50 dark:bg-red-900/10 text-red-600 border-red-200' : 'bg-gray-100 dark:bg-darkInput text-gray-400 border-gray-200 dark:border-darkBorder'}`}>
              ...
              <button onClick={handleMainButton} className={`flex items-center gap-2 px-4 py-3 rounded-lg text-white text-sm font-black shadow-md uppercase ${btnState.color} transition-all`}>
      ...
      <div className="max-w-4xl mx-auto p-4 flex flex-col gap-4">
          <div className="relative w-full aspect-[16/8] bg-green-600 rounded-xl overflow-hidden border-4 border-green-800 shadow-inner select-none">
          ...
          {isRunning ? (
              <div className="space-y-4 animate-fade-in">
                  {step === 0 && (
                      <div className="grid grid-cols-4 gap-2">
                          {(['OFENSIVA', 'DEFENSIVA', 'TRANSICAO_OF', 'TRANSICAO_DEF'] as GamePhase[]).map(phase => (
                              <button key={phase} onClick={() => setActivePhase(phase)} className={`py-4 rounded-xl text-[9px] md:text-[10px] font-black uppercase transition-all shadow-sm border-2 flex flex-col items-center justify-center gap-1 leading-tight ${activePhase === phase ? 'bg-indigo-600 text-white border-indigo-400 scale-105 shadow-xl' : 'bg-white dark:bg-darkCard text-gray-400 border-gray-100 dark:border-darkBorder'}`}>
                  ...
                  {step > 0 && (
                      <div className="bg-white dark:bg-darkCard rounded-2xl shadow-xl border-2 border-blue-100 dark:border-darkBorder overflow-hidden animate-slide-up">
                          <div className="bg-blue-600 text-white px-4 py-3 flex justify-between items-center">
                              <h3 className="font-black uppercase tracking-widest text-xs">{step === 1 ? `AÇÃO - ${activePhase.replace('_', ' ')}` : 'RESULTADO'}</h3>
                              ...
                              {step === 1 && (
                                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                      {PHASE_ACTIONS[activePhase].map(action => (
                                          <button key={action} onClick={() => handleActionClick(action)} className="py-5 px-2 rounded-xl font-black text-xs bg-gray-50 dark:bg-darkInput text-gray-800 dark:text-gray-200 border-2 border-gray-100 dark:border-darkBorder hover:border-blue-400 uppercase tracking-widest">{action}</button>
                                      ))}
                                  </div>
                              )}
                              {step === 2 && (
                                  <div className="grid grid-cols-3 gap-3">
                                      <button onClick={() => handleResultClick('POSITIVA')} className="bg-green-600 text-white py-10 rounded-2xl font-black text-xl border-b-4 border-green-800 uppercase tracking-widest">POSITIVA</button>
                                      <button onClick={() => handleResultClick('NEUTRA')} className="bg-gray-500 text-white py-10 rounded-2xl font-black text-xl border-b-4 border-gray-700 uppercase tracking-widest">NEUTRA</button>
                                      <button onClick={() => handleResultClick('NEGATIVA')} className="bg-red-600 text-white py-10 rounded-2xl font-black text-xl border-b-4 border-red-800 uppercase tracking-widest">NEGATIVA</button>
                                  </div>
                              )}
                              <button onClick={() => {setStep(0); setCapturedLocation(null);}} className="w-full py-3 text-gray-400 font-black text-[10px] uppercase bg-gray-50 dark:bg-darkInput rounded-xl hover:text-red-500 transition-colors tracking-widest">Cancelar Registro</button>
                          </div>
                      </div>
                  )}
                  {step === 0 && (
                      <div className="grid grid-cols-3 gap-3">
                          <div className="bg-green-50 dark:bg-emerald-900/10 p-4 rounded-2xl border-2 border-green-100 dark:border-emerald-800/30 text-center"><span className="text-3xl font-black text-green-700 dark:text-emerald-400">{currentEvents.filter(e => e.result === 'POSITIVA').length}</span><p className="text-[10px] font-black text-green-600 uppercase tracking-widest">Positivas</p></div>
                          <div className="bg-gray-100 dark:bg-darkInput p-4 rounded-2xl border-2 border-gray-200 dark:border-darkBorder text-center"><span className="text-3xl font-black text-gray-700 dark:text-gray-300">{currentEvents.filter(e => e.result === 'NEUTRA').length}</span><p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Neutras</p></div>
                          <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-2xl border-2 border-red-100 dark:border-red-800/30 text-center"><span className="text-3xl font-black text-red-700 dark:text-red-400">{currentEvents.filter(e => e.result === 'NEGATIVA').length}</span><p className="text-[10px] font-black text-red-600 uppercase tracking-widest">Negativas</p></div>
                      </div>
                  )}
              </div>
          ) : (
            <div className="bg-white dark:bg-darkCard rounded-2xl p-10 text-center border-2 border-dashed border-gray-200 dark:border-darkBorder">
                <Activity size={48} className="mx-auto text-blue-200 dark:text-gray-700 mb-4" />
                <h2 className="text-2xl font-black text-gray-800 dark:text-gray-100 uppercase tracking-tighter">Sessão em Espera</h2>
                <p className="text-gray-400 text-sm mt-2 max-w-xs mx-auto font-medium">Toque em "Iniciar" para monitorar as ações táticas. Inclua mais atletas abaixo para monitoramento múltiplo.</p>
            </div>
          )}
      </div>

      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-darkCard border-t border-gray-200 dark:border-darkBorder p-4 shadow-2xl z-40 flex flex-col md:flex-row gap-4 md:items-center md:px-8">
          <div className="flex items-center gap-4 w-full md:w-auto">
              <button onClick={() => setShowCancelModal(true)} className="text-red-500 dark:text-red-400 font-black text-[10px] uppercase tracking-widest flex items-center gap-2 px-3 py-2 hover:bg-red-50 rounded-lg"><XCircle size={18} /> Cancelar</button>
              <button onClick={() => setShowAddAthleteModal(true)} className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 p-3 rounded-full border border-blue-200 dark:border-blue-800 hover:scale-110 transition-transform"><UserPlus size={20} /></button>
          </div>
          <div className="flex-1 overflow-x-auto flex gap-3 pb-1 hide-scrollbar">
              {activeAthletes.map(ath => (
                  <button key={ath.id} onClick={() => setSelectedAthleteId(ath.id)} className={`flex items-center gap-2 p-2 rounded-lg border transition-all min-w-[140px] ${selectedAthleteId === ath.id ? 'bg-blue-600 text-white border-blue-700 shadow-md' : 'bg-white dark:bg-darkInput text-gray-700 dark:text-gray-300 border-gray-200 dark:border-darkBorder hover:bg-gray-50'}`}>
                      {ath.photoUrl ? <img src={ath.photoUrl} className="w-8 h-8 rounded-full object-cover" /> : <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-[10px] ${selectedAthleteId === ath.id ? 'bg-white/20' : 'bg-blue-100 text-blue-600'}`}>{ath.name.charAt(0)}</div>}
                      <div className="flex flex-col items-start min-w-0"><span className="text-xs font-black truncate w-full uppercase tracking-tighter">{ath.name.split(' ')[0]}</span><span className={`text-[10px] font-bold ${selectedAthleteId === ath.id ? 'text-blue-100' : 'text-gray-400'}`}>{sessionLogs[ath.id]?.length || 0} ações</span></div>
                  </button>
              ))}
          </div>
          <button onClick={() => setShowFinishModal(true)} disabled={Object.values(sessionLogs).every(l => l.length === 0)} className="bg-gray-900 dark:bg-emerald-600 text-white font-black py-4 px-8 rounded-2xl shadow-xl flex items-center gap-2 hover:bg-black uppercase tracking-widest text-xs w-full md:w-auto justify-center border-b-4 border-black dark:border-emerald-800">
              <StopCircle size={20} /> Salvar Atuação
          </button>
      </div>

      {/* MODAL INCLUIR ATLETA */}
      {showAddAthleteModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white dark:bg-darkCard dark:border dark:border-darkBorder rounded-3xl w-full max-w-lg p-6 shadow-2xl animate-slide-up">
                  <div className="flex justify-between items-center mb-6">
                      <h3 className="text-xl font-black text-gray-800 dark:text-gray-100 uppercase tracking-tighter flex items-center gap-2"><UserPlus className="text-blue-600" /> Incluir no Monitoramento</h3>
                      <button onClick={() => setShowAddAthleteModal(false)} className="p-2 text-gray-400 hover:text-red-500"><X size={24}/></button>
                  </div>
                  <div className="relative mb-6">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18}/>
                      <input type="text" placeholder="Pesquisar por nome ou RG..." className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-darkInput border border-gray-100 dark:border-darkBorder rounded-2xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}/>
                  </div>
                  <div className="max-h-[350px] overflow-y-auto space-y-2 custom-scrollbar pr-2">
                      {filteredSearchAthletes.map(ath => (
                          <div key={ath.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-darkInput/50 rounded-2xl border border-gray-100 dark:border-darkBorder group hover:border-blue-200">
                              <div className="flex items-center gap-3">
                                  {ath.photoUrl ? <img src={ath.photoUrl} className="w-10 h-10 rounded-full object-cover" /> : <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center font-bold text-blue-600 text-xs">{ath.name.charAt(0)}</div>}
                                  <div><p className="text-sm font-black text-gray-800 dark:text-gray-100 uppercase truncate">{ath.name}</p><p className="text-[10px] text-gray-400 font-bold uppercase">{ath.position} • {ath.rg}</p></div>
                              </div>
                              <button onClick={() => handleAddAthlete(ath)} className="p-2 bg-blue-600 text-white rounded-xl shadow-md hover:bg-blue-700 active:scale-95 transition-all"><Plus size={18}/></button>
                          </div>
                      ))}
                      {filteredSearchAthletes.length === 0 && <div className="p-10 text-center text-gray-400 font-bold italic uppercase text-xs">Nenhum atleta disponível para seleção.</div>}
                  </div>
              </div>
          </div>
      )}
      ...
      {showFinishModal && (
         <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
             <div className="bg-white dark:bg-darkCard border dark:border-darkBorder rounded-3xl w-full max-w-sm p-8 shadow-2xl text-center animate-slide-up">
                 ...
                 <h2 className="text-2xl font-black text-gray-800 dark:text-gray-100 mb-2 uppercase tracking-tighter">Finalizar Coleta?</h2>
                 <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm leading-relaxed">Você registrará o desempenho de <strong>{activeAthletes.length} atletas</strong>. O algoritmo processará todos os eventos táticos capturados nesta sessão.</p>
                 <div className="flex flex-col gap-3">
                     <button onClick={handleFinishSession} className="w-full bg-blue-600 dark:bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-xs border-b-4 border-blue-800 dark:border-indigo-800 active:scale-95">Confirmar e Processar Scores</button>
      ...
      {showCancelModal && (
         <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
             <div className="bg-white dark:bg-darkCard border dark:border-darkBorder rounded-3xl w-full max-w-sm p-8 shadow-2xl text-center animate-slide-up">
                 ...
                 <h2 className="text-2xl font-black text-gray-800 dark:text-gray-100 mb-2 uppercase tracking-tighter">Descartar Sessão?</h2>
                 <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm leading-relaxed">Toda a cronometragem e eventos registrados serão permanentemente apagados.</p>
                 <div className="flex flex-col gap-3">
                     <button onClick={() => navigate(`/athletes/${id}`)} className="w-full bg-red-600 text-white font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-xs border-b-4 border-red-900">Sim, Descartar</button>
                     <button onClick={() => setShowCancelModal(false)} className="w-full bg-gray-100 dark:bg-darkInput text-gray-500 dark:text-gray-400 font-black py-3 rounded-2xl text-xs uppercase tracking-widest">Voltar ao Monitoramento</button>
                 </div>
             </div>
         </div>
      )}

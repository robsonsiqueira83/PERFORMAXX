...
import { 
  getAthletes, getCategories, saveTrainingEntry, saveTrainingSession, getTrainingSessions, getEvaluationSessions
} from '../services/storageService';
import { Athlete, Category, TrainingEntry, TrainingSession, Position, User, canEditData, EvaluationSession } from '../types';
import { Save, Users, ClipboardList, Loader2, Search, Target, AlertOctagon, Activity } from 'lucide-react';
...
const Training: React.FC<TrainingProps> = ({ teamId }) => {
  ...
  const [allEvals, setAllEvals] = useState<EvaluationSession[]>([]);
  ...
  useEffect(() => {
    ...
    const init = async () => {
        setLoading(true);
        const [c, a, ev] = await Promise.all([
            getCategories(), 
            getAthletes(),
  ...
  const handleSaveEntry = async () => {
    ...
    const entry: TrainingEntry = {
      ...
      technical: {
        controle_bola: scores['Passe|Longo'] || avgTech,
  ...
  const getAthleteAverage = (athleteId: string) => {
      const athleteEvals = allEvals.filter(ev => ev.athleteId === athleteId);
      if (athleteEvals.length === 0) return 0;
      return athleteEvals.reduce((acc, curr) => acc + curr.scoreTecnico, 0) / athleteEvals.length;
  };
      ...
      {selectedCategory && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 overflow-x-auto">
            ...
            <div className="flex gap-4 pb-2">
                {athletes.map(a => {
                    const avg = getAthleteAverage(a.id);
                    return (
                    <button key={a.id} onClick={() => handleSelectAthlete(a)} className={`flex-shrink-0 flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all relative ${selectedAthlete?.id === a.id ? 'bg-blue-600 text-white border-blue-700 shadow-md scale-105' : 'bg-gray-50 text-gray-700 border-gray-100 hover:bg-blue-50'}`}>
                        <div className="relative">
                            ...
                            {avg > 0 && <div className="absolute -top-1 -right-1 bg-emerald-500 text-white text-[8px] font-black px-1 rounded border border-white shadow-sm">{avg.toFixed(1)}</div>}
                        </div>
                        <div className="text-center">
                            <span className="text-[9px] font-black uppercase tracking-tighter truncate block max-w-[80px] leading-none">{a.name.split(' ')[0]}</span>
                            <span className={`text-[7px] font-black uppercase mt-0.5 block opacity-60`}>{a.position}</span>
                        </div>
                    </button>
                )})}
            </div>

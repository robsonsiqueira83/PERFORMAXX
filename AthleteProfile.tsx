...
const AthleteProfile: React.FC = () => {
  ...
  const handleEditDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (newDate && athlete) {
         updateCategoryForEdit(newDate, athlete.teamId);
      }
  };

  const updateCategoryForEdit = async (date: string, teamId: string) => {
       const calcName = getCalculatedCategory(date);
       if (calcName) {
           const cat = await ensureCategoryExists(teamId, calcName);
           const updatedCats = await getCategories();
           setCategories(updatedCats.filter(c => c.teamId === teamId));
